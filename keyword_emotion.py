...
def sort_keyword_byallemotion(result_dic,islog = 0):
    output_dic = {}
    temp_dic = {}
    output_dic['keyword'] = []
    output_dic['log'] = []
    ...
    for current_keyword, current_keyword_list in result_dic.items():
        if(islog):
            temp_dic[current_keyword] = sum_all(current_keyword_list,1)
        else:
            temp_dic[current_keyword] = 1
...
def trans_result2Percent(input_data):
    output_dic = {}
    for current_keyword,current_keyword_list in input_data.items():
        output_dic[current_keyword] = {}
        sum = sum_all(current_keyword_list)
        for current_keyword_emotion,emotion_count in current_keyword_list.items():
            output_dic[current_keyword][current_keyword_emotion] = float(emotion_count) / sum
    return output_dic

def to_echartsData_percent(result_dic, emotion_list, sorted_keywords_list):
    output_dic = {}
    # 对数量先进行排序
...
if __name__=='__main__':
     ...
     result_dic = get_result_dic_2()
     fw = open("data/city_keyword_emotion/result.txt", 'w+')
     fw.write(str(result_dic))  # 把字典转化为str
     fw.close()
     ...
     #
     # sorted_keywords_dic = sort_keyword_byallemotion(result_dic,1)
     ...
     # to_echartsData_percent(result_dic,emotion_list,sorted_keywords_dic['keyword'])
